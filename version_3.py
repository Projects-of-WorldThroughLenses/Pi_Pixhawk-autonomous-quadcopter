#!/usr/bin/env python3
"""
Medical Delivery Drone GCS v60 Light  —  PyQt5 Edition
====================================================
v60 changes (from v50):
  • SERVO IS NOW DRIVEN VIA PIXHAWK, NOT PI GPIO.
    The payload servo is wired to Pixhawk SERVO9 with
    SERVO9_FUNCTION = RCIN11. The Pi controls it the same way it
    controls the sticks: by sending RC_CHANNELS_OVERRIDE on RC ch11.
    DirectServoController and all RPi.GPIO/pigpio code has been removed.
  • SERVO AUTHORITY follows the SAME ch9 override switch as the sticks:
      switch LOW  → Pi sends NO_OVERRIDE on ch11 → TX ch11 → RCIN11
                    → SERVO9 (TX has full servo authority).
      switch HIGH → Pi overrides ch11 directly with the commanded PWM
                    (Pi has full servo authority, exactly like ch1/2/4).
  • NEW SERVO PWM ENDPOINTS: OPEN=2000, MID=1495, CLOSE=999.
  • KEYBOARD (only when ch9 HIGH):
      8 → snap to 1495 (mid)
      9 → ramp DOWN slowly, floor 999
      0 → ramp UP   slowly, ceiling 2000
  • UI BUTTONS in Dashboard, GPS (Map), and Dead Reckoning tabs:
      Open / Mid / Close — disabled while ch9 LOW (TX has authority).
  • TAKEOFF ALTITUDE GATE (new in this revision):
      Both GPS and DR missions are now set up on the ground before
      takeoff. Clicking FLY TO TARGET / START DEAD RECKONING no longer
      starts horizontal flight immediately. Instead the Pi enters a
      WAITING FOR TAKEOFF state in which ch1/ch2/ch4 are held at NEUTRAL
      (keeping the drone level) while ch3 throttle stays TX-controlled
      so the pilot can lift off vertically. Once the Pixhawk's relative
      altitude (barometer-fused) reaches TAKEOFF_ALT_M (4 m), the Pi
      snapshots the current GPS position as the return-to-base point
      and the current servo PWM as the pre-flight servo value, then
      engages the real GPS / DR go-to loop.
  • IN-FLIGHT AUTO-DROP SEQUENCE:
      On reaching the GPS or DR target, the Pi commands servo to 2000
      (open), holds for 2 seconds, restores the pre-flight servo PWM,
      and then begins the return leg back to the take-off point.
  • Older v50 notes preserved below.
  ----
...
BEFORE RUNNING:
  ...
  pip install opencv-python-headless PyQt5 PyQtWebEngine pymavlink picamera2
"""
...
# When the operator clicks FLY TO TARGET or START DEAD RECKONING on the
# ground, the Pi enters a waiting state that holds the sticks at NEUTRAL
# (letting the pilot lift off vertically using TX throttle). The real
# GPS/DR go-to loop only engages once the Pixhawk's relative altitude
# (barometer-fused) reaches this value in metres.
TAKEOFF_ALT_M = 4.0
...
class GCSWindow(QMainWindow):
    def __init__(self, S: SharedState):
        ...
        self.setWindowTitle("Medical Delivery Drone GCS v60 Light  |  Pi local go-to  |  servo via Pixhawk RC ch11")
    ...
    def _build_ui(self):
        ...
        hint = QLabel(
            ...
            f"TX ch{OVERRIDE_SWITCH_CH} HIGH → Pi controls sticks + servo (ch{SERVO_OUTPUT_CH})")
    ...
    def start_gps_autopilot(self):
        ...
        with self.S.lock:
            ...
            # That way return-to-base returns to the actual lift-off spot
            # and the servo restore uses whatever the pilot had dialled in
            # right before leaving the ground.
            self.S.mission_start_lat = None
            self.S.mission_start_lon = None
            ...
            self.S.ap_status = (
                ...
                f"waiting for takeoff ({TAKEOFF_ALT_M:.0f} m)")
    ...
    def _tick_ap(self):
        with self.S.lock:
            ...
            sw_on = self.S.override_active
            waiting_takeoff = self.S.ap_waiting_for_takeoff
        ...
        # ── Takeoff altitude gate ─────────────────────────────
        # While waiting for the drone to reach TAKEOFF_ALT_M, hold
        # ch1/ch2/ch4 at NEUTRAL so the pilot can vertical-takeoff
        # using TX throttle (ch3 is never overridden). The gate opens
        # the moment rel_alt first crosses the threshold, at which
        # point we capture the return-to-base point and the pre-flight
        # servo PWM and fall through to the normal AP loop.
        if waiting_takeoff:
            with self.S.lock:
                rel_alt = self.S.tel.get("rel_alt_m")
                cur_lat = self.S.tel.get("lat")
                cur_lon = self.S.tel.get("lon")
                target_lat_gps = self.S.ap_target_lat
                target_lon_gps = self.S.ap_target_lon
                dr_bearing = self.S.dr_target_bearing
                dr_dist = self.S.dr_target_dist_m
                self.S.ap_roll_pwm = NEUTRAL
                self.S.ap_pitch_pwm = NEUTRAL
                self.S.ap_yaw_pwm = NEUTRAL
            ...
            if rel_alt is None:
                with self.S.lock:
                    self.S.ap_status = (
                        f"Waiting for barometer...  "
                        f"(need {TAKEOFF_ALT_M:.1f} m)")
                return
            ...
            if rel_alt < TAKEOFF_ALT_M:
                with self.S.lock:
                    if ap_mode == APMode.GPS:
                        self.S.ap_status = (
                            f"Waiting for takeoff  |  "
                            f"{rel_alt:.1f} / {TAKEOFF_ALT_M:.0f} m  |  "
                            f"target {target_lat_gps:.5f},{target_lon_gps:.5f}"
                            if target_lat_gps is not None else
                            f"Waiting for takeoff  |  {rel_alt:.1f} / {TAKEOFF_ALT_M:.0f} m")
                    else:
                        self.S.ap_status = (
                            f"Waiting for takeoff  |  "
                            f"{rel_alt:.1f} / {TAKEOFF_ALT_M:.0f} m  |  "
                            f"DR {dr_bearing:.0f}° {dr_dist:.1f} m")
                return

            # Altitude reached — latch the mission.
            with self.S.lock:
                self.S.ap_waiting_for_takeoff = False
                # Return-to-base: where the drone is RIGHT NOW (which is
                # directly above the lift-off spot for a vertical takeoff).
                if cur_lat is not None and cur_lon is not None and cur_lat != 0.0:
                ...
                # Pre-flight servo snapshot = whatever the pilot has set
                # right up until the gate opens.
                self.S.servo_preflight_pwm = int(self.S.servo_target_pwm)
                # Reset the DR "last update" clock so dt doesn't include
                # the entire ground-and-takeoff wait period.
                self.S.dr_last_update = time.time()
                ...
                if ap_mode == APMode.GPS:
                    self.S.ap_status = (
                        f"Takeoff confirmed ({rel_alt:.1f} m)  |  "
                        f"GPS go-to  |  return {ms_lat:.5f},{ms_lon:.5f}  |  "
                        f"servo restore {preflight}"
                        if ms_lat is not None else
                        f"Takeoff confirmed ({rel_alt:.1f} m)  |  GPS go-to")
                else:
                    self.S.ap_status = (
                        f"Takeoff confirmed ({rel_alt:.1f} m)  |  "
                        f"DR align {dr_bearing:.0f}° / {dr_dist:.1f} m  |  "
                        f"servo restore {preflight}")
            # Fall through to the normal GPS/DR logic on the next tick.
            return
    ...
    # ══════════════════════════════════════════════════════════
    #  RC OVERRIDE SEND  (20 Hz, local stick simulation + servo)
    #
    #  ch3 (throttle) = NO_OVERRIDE always — TX owns throttle.
    #  ch11 (servo via Pixhawk SERVO9_FUNCTION=RCIN11) is now sent by
    #  the Pi the same way ch1/ch2/ch4 are: only while the override
    #  switch (ch9) is HIGH. While the switch is LOW, the Pi sends
    #  NO_OVERRIDE on ch11 so the TX retains full servo authority.
    #
    #  Switch OFF → all NO_OVERRIDE (sticks AND servo).
    #  Switch ON  + AP → computed AP stick values + commanded servo PWM.
    #  Switch ON  + keyboard/idle → smoothed local sticks + commanded servo PWM.
    #  Locked → NEUTRAL on ch1/ch2/ch4; servo follows commanded value.
    ...
    def _tick_send(self):
        ...
        with self.S.lock:
            ...
            if not sw_on:
                ...
                servo_pwm = NO_OVERRIDE  # TX has servo authority via RCIN11
        ...
        try:
            self.m.mav.rc_channels_override_send(
                ...
                chan9_raw=NO_OVERRIDE,
                chan10_raw=NO_OVERRIDE,
                chan11_raw=int(servo_pwm),
                chan12_raw=NO_OVERRIDE,
                chan13_raw=NO_OVERRIDE,
                chan14_raw=NO_OVERRIDE,
                chan15_raw=NO_OVERRIDE,
                chan16_raw=NO_OVERRIDE,
    ...
    def _tick_ui(self):
        ...
        elif ap_active and ap_waiting_takeoff:
            self.lbl_state.setText(f"WAITING FOR TAKEOFF ({TAKEOFF_ALT_M:.0f} m)")
            self.lbl_state.setStyleSheet(
