"""
Drone Dashboard v101 Optimized FPV - based on v100 Live/Post Computer Vision + Low-Latency Split FPV GCS
...
# OpenCV capture tuning for Windows. This can reduce periodic MSMF capture stalls on some USB devices.
...
class AISettings:
    ...
    ai_half: bool = True
    ai_max_det: int = 50
    ai_input_width: int = 640              # downscale frame for inference then scale boxes back
    ai_async: bool = True
...
class AIFrameThread(QThread):
    ...
    def get_latest_frame_and_detections(self):
        ...
        with self._frame_lock:
            ...
            # v101 FPV optimization:
            # Do NOT copy the full video frame here. The display widget keeps a reference
            # to the numpy frame long enough for QPainter to draw it. This removes one
            # large memory copy every UI refresh and helps prevent periodic GC/memory stalls.
    ...
    def start_fpv_recording(self, base_dir: str, fps: Optional[float] = None) -> str:
        ...
        stamp = time.strftime(f'fpv_{int(self._record_fps)}fps_%Y%m%d_%H%M%S')
    ...
    def run(self):
        ...
        # v101 FPV optimization:
        # Disable Python cyclic GC during the live capture loop. The FPV path creates many
        # short-lived objects at 60 FPS; periodic GC checks can appear as regular stutters.
        # Reference counting still frees normal objects. GC is restored when FPV stops.
        ...
        try:
            ...
            pass
        try:
            if '_gc_was_enabled' in locals() and _gc_was_enabled:
                gc.enable()
        except Exception:
            pass
...
class FpvOverlayView(QWidget):
    ...
    def __init__(self):
        ...
        self._qimage = None
        self._frame_ref = None  # keeps numpy memory alive for zero-copy QImage display
        self._img_w = 0
        self._img_h = 0
    ...
    def update_frame(self, frame_bgr: np.ndarray, detections: List[Dict[str, Any]], fps: float = 0.0):
        if frame_bgr is None:
            return
        # v101 FPV optimization:
        # Use a zero-copy QImage path. The older version called QImage.copy() every frame,
        # which created a full extra frame allocation and caused periodic memory/GC stalls.
        # Keeping self._frame_ref alive makes the QImage data pointer safe until the next frame.
        ...
        try:
            if fmt == QImage.Format_RGB888:
                ...
                self._frame_ref = rgb
                self._qimage = QImage(self._frame_ref.data, w, h, self._frame_ref.strides[0], QImage.Format_RGB888)
            else:
                self._frame_ref = frame_bgr
                self._qimage = QImage(self._frame_ref.data, w, h, self._frame_ref.strides[0], fmt)
        except Exception:
            rgb = cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2RGB)
            self._frame_ref = rgb
            self._qimage = QImage(self._frame_ref.data, w, h, self._frame_ref.strides[0], QImage.Format_RGB888)
...
class FPVAIWidget(QGroupBox):
    ...
    def update_frame(self, frame: np.ndarray, detections: List[Dict[str, Any]], display_fps: float = 0.0):
        ...
        # v101 FPV optimization:
        # Updating QLabel text every video frame can trigger layout/style recalculation.
        # Throttle the small summary label to 4 Hz so the video panel stays smooth.
        now = time.time()
        if now - self._last_summary_update < 0.25:
            return
        ...
        if counts:
            ...
            new_text = f"AI boxes overlaid on smooth FPV: {txt}"
        else:
            new_text = "AI boxes: none | FPV path remains separate from AI path"
        if new_text != self._last_summary_text:
            self.summary.setText(new_text)
            self._last_summary_text = new_text
...
class CompactMissionPlannerWidget(QGroupBox):
    ...
    def __init__(self):
        ...
        self.start_lat = None
        self.start_lon = None
        # v102: throttle WebEngine GPS updates. Calling runJavaScript on every MAVLink
        # packet can stall the Qt UI/FPV view periodically. Keep the map live but limit
        # it to a smooth 5 Hz and only send meaningful changes.
        self._last_map_update_t = 0.0
        self._last_map_lat = None
        self._last_map_lon = None
        self._last_map_hdg = None
        ...
        if WEBENGINE_AVAILABLE:
            ...
            self.map.setHtml(self._map_html(), QUrl("qrc:///"))
            try:
                self.map.loadFinished.connect(lambda ok: setattr(self, '_map_loaded', bool(ok)))
            except Exception:
                pass
    ...
    def _map_html(self):
        # v112: robust GPS map.
        # Leaflet/OSM is used when available. If the CDN is unavailable or WebEngine blocks
        # the external JS, it automatically falls back to an offline canvas radar map. Both
        # modes expose updateDrone() and setWaypoint(), so Python-side GPS updates keep working.
        return """
<!DOCTYPE html><html><head><meta charset='utf-8'/>
<style>
html,body,#map{height:100%;margin:0;background:#07111f;overflow:hidden;font-family:Arial,Helvetica,sans-serif}
.status{position:absolute;z-index:999;left:10px;top:10px;background:rgba(3,7,18,.82);color:white;padding:7px 9px;border:1px solid rgba(148,163,184,.45);border-radius:10px;font-size:12px;box-shadow:0 4px 16px rgba(0,0,0,.4)}
.fallbackBadge{position:absolute;z-index:999;right:10px;bottom:10px;background:rgba(15,23,42,.72);color:#bfdbfe;border:1px solid rgba(96,165,250,.55);border-radius:10px;padding:5px 8px;font-size:11px}
canvas{width:100%;height:100%;display:block;background:radial-gradient(circle at center,#10294a 0%,#071827 42%,#020617 100%)}
</style>
<link rel='stylesheet' href='https://unpkg.com/leaflet@1.9.4/dist/leaflet.css'/>
<script src='https://unpkg.com/leaflet@1.9.4/dist/leaflet.js'></script>
<script src='qrc:///qtwebchannel/qwebchannel.js'></script></head><body>
<div id='map'></div><div class='status' id='status'>Waiting for GPS...</div><div class='fallbackBadge' id='badge'>Map loading</div>
<script>
var bridge=null;
try { new QWebChannel(qt.webChannelTransport,function(channel){bridge=channel.objects.bridge;}); } catch(e) {}
var map=null, drone=null, waypoint=null, line=null, useLeaflet=false, firstDroneFix=true;
var current={lat:1.3521,lon:103.8198,hdg:0,sats:0,alt:0};
var wp=null, canvas=null, ctx=null;
function safeNum(v,d){v=Number(v); return isFinite(v)?v:d;}
function statusText(){document.getElementById('status').innerHTML='Drone '+current.lat.toFixed(6)+', '+current.lon.toFixed(6)+'<br>HDG '+current.hdg.toFixed(0)+'° | Sats '+current.sats+' | Alt '+current.alt.toFixed(1)+'m';}
function droneSvg(){return '<div class="drone-arrow" style="width:36px;height:36px;transform-origin:18px 18px;filter:drop-shadow(0 2px 5px rgba(0,0,0,.65));"><svg width="36" height="36" viewBox="0 0 36 36"><path d="M18 2 L29 32 L18 25 L7 32 Z" fill="#2563eb" stroke="white" stroke-width="2.4"/><path d="M18 6 L22 23 L18 20 L14 23 Z" fill="#93c5fd" opacity="0.95"/><circle cx="18" cy="18" r="4.5" fill="#ef4444" stroke="white" stroke-width="1.4"/></svg></div>';}
function updateLeafletIcon(){var el=drone?drone.getElement():null; if(el){var arrow=el.querySelector('.drone-arrow'); if(arrow){arrow.style.transform='rotate('+current.hdg.toFixed(1)+'deg)';}}}
function initLeaflet(){
  if(typeof L==='undefined'){initFallback('Offline GPS canvas'); return;}
  try{
    useLeaflet=true; document.getElementById('badge').innerHTML='Leaflet / OSM';
    map=L.map('map',{zoomControl:false,preferCanvas:true}).setView([current.lat,current.lon],17);
    L.control.zoom({position:'bottomright'}).addTo(map);
    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:20,updateWhenIdle:true,updateWhenZooming:false,keepBuffer:1,attribution:'&copy; OSM'}).addTo(map);
    var droneIcon=L.divIcon({className:'drone-div-icon',html:droneSvg(),iconSize:[36,36],iconAnchor:[18,18]});
    drone=L.marker([current.lat,current.lon],{icon:droneIcon,interactive:false}).addTo(map);
    map.on('click',function(e){setWaypoint(e.latlng.lat,e.latlng.lng); if(bridge){bridge.mapClicked(e.latlng.lat,e.latlng.lng);}});
    statusText(); updateLeafletIcon();
  }catch(e){initFallback('Map fallback');}
}
function initFallback(label){
  useLeaflet=false; document.getElementById('badge').innerHTML=label||'Offline GPS canvas';
  var m=document.getElementById('map'); m.innerHTML='<canvas id="radar"></canvas>';
  canvas=document.getElementById('radar'); ctx=canvas.getContext('2d');
  function resize(){canvas.width=Math.max(320,m.clientWidth); canvas.height=Math.max(220,m.clientHeight); drawFallback();}
  window.addEventListener('resize',resize); resize();
  canvas.addEventListener('click',function(ev){
    var r=canvas.getBoundingClientRect(); var x=ev.clientX-r.left, y=ev.clientY-r.top;
    // approximate click as local meter offset around current GPS
    var scale=0.0000065; var dx=(x-r.width/2)*scale; var dy=-(y-r.height/2)*scale;
    setWaypoint(current.lat+dy,current.lon+dx); if(bridge){bridge.mapClicked(current.lat+dy,current.lon+dx);}
  });
  statusText(); drawFallback();
}
function drawFallback(){
  if(!ctx||!canvas)return; var w=canvas.width,h=canvas.height,cx=w/2,cy=h/2;
  ctx.clearRect(0,0,w,h);
  var g=ctx.createRadialGradient(cx,cy,20,cx,cy,Math.max(w,h)/1.2); g.addColorStop(0,'#163b63'); g.addColorStop(0.55,'#071827'); g.addColorStop(1,'#020617'); ctx.fillStyle=g; ctx.fillRect(0,0,w,h);
  ctx.strokeStyle='rgba(96,165,250,.22)'; ctx.lineWidth=1;
  for(var r=40;r<Math.max(w,h);r+=40){ctx.beginPath();ctx.arc(cx,cy,r,0,Math.PI*2);ctx.stroke();}
  for(var a=0;a<360;a+=30){var rad=a*Math.PI/180;ctx.beginPath();ctx.moveTo(cx,cy);ctx.lineTo(cx+Math.cos(rad)*w,cy+Math.sin(rad)*h);ctx.stroke();}
  ctx.save(); ctx.translate(cx,cy); ctx.rotate(current.hdg*Math.PI/180); ctx.fillStyle='#2563eb'; ctx.strokeStyle='white'; ctx.lineWidth=2; ctx.beginPath(); ctx.moveTo(0,-24); ctx.lineTo(14,20); ctx.lineTo(0,11); ctx.lineTo(-14,20); ctx.closePath(); ctx.fill(); ctx.stroke(); ctx.restore();
  ctx.fillStyle='#ef4444'; ctx.beginPath(); ctx.arc(cx,cy,5,0,Math.PI*2); ctx.fill();
  if(wp){var scale=150000; var px=cx+(wp.lon-current.lon)*scale; var py=cy-(wp.lat-current.lat)*scale; ctx.strokeStyle='#22c55e'; ctx.lineWidth=3; ctx.beginPath();ctx.moveTo(cx,cy);ctx.lineTo(px,py);ctx.stroke(); ctx.fillStyle='#22c55e'; ctx.beginPath();ctx.arc(px,py,7,0,Math.PI*2);ctx.fill();}
  ctx.fillStyle='rgba(255,255,255,.9)'; ctx.font='12px Arial'; ctx.fillText('LAT '+current.lat.toFixed(6)+'  LON '+current.lon.toFixed(6),12,h-16);
}
function updateDrone(lat,lon,hdg,sats,alt){
  lat=safeNum(lat,null); lon=safeNum(lon,null); if(lat===null||lon===null)return;
  current.lat=lat; current.lon=lon; current.hdg=safeNum(hdg,0); current.sats=parseInt(safeNum(sats,0)); current.alt=safeNum(alt,0); statusText();
  if(useLeaflet && drone){
    drone.setLatLng([lat,lon]); updateLeafletIcon();
    if(firstDroneFix){map.setView([lat,lon],17,{animate:false}); firstDroneFix=false;}
    if(line&&waypoint){line.setLatLngs([drone.getLatLng(),waypoint.getLatLng()]);}
  } else { drawFallback(); }
}
function setWaypoint(lat,lon){
  lat=safeNum(lat,null); lon=safeNum(lon,null); if(lat===null||lon===null)return; wp={lat:lat,lon:lon};
  if(useLeaflet && map){ if(waypoint){waypoint.setLatLng([lat,lon]);} else {waypoint=L.marker([lat,lon],{draggable:true}).addTo(map).bindPopup('Waypoint'); waypoint.on('dragend',function(e){var p=e.target.getLatLng(); if(bridge){bridge.mapClicked(p.lat,p.lng);}});} if(line){map.removeLayer(line);} line=L.polyline([drone.getLatLng(),[lat,lon]],{color:'#22c55e',weight:4,opacity:.9}).addTo(map); }
  else { drawFallback(); }
}
setTimeout(function(){ if(!useLeaflet && !canvas){initFallback('Offline GPS canvas');}},1600);
...
"""
    ...
    def update_telemetry(self, d: Dict[str, Any]):
        ...
        try:
            ...
            return
        hdg = safe_float(self.telemetry.get("heading_deg")); sats = int(self.telemetry.get("gps_sats", 0)); alt = safe_float(self.telemetry.get("rel_alt_m"))
        now = time.time()
        try:
            # Text label is cheap; keep it current even if WebEngine is not ready.
            if now - getattr(self, "_last_map_text_t", 0.0) >= 0.25:
                self._last_map_text_t = now
                self.map_status.setText(f"Drone GPS: {latf:.6f}, {lonf:.6f} | HDG {hdg:.0f}° | Sats {sats} | Alt {alt:.1f}m")
        except Exception:
            pass
        if not (WEBENGINE_AVAILABLE and hasattr(self, "map") and isinstance(self.map, QWebEngineView)):
            return
        # v112: WebEngine updates are the usual source of the tiny FPV hitch. Keep map
        # updates stable at ~2 Hz and do not pan the map every packet; JavaScript only
        # moves/rotates the drone marker after the first GPS fix.
        if now - getattr(self, "_last_map_update_t", 0.0) < 0.50:
            return
        self._last_map_update_t = now
        self._last_map_lat = latf; self._last_map_lon = lonf; self._last_map_hdg = hdg
        try:
            js = f"try{{updateDrone({latf:.8f},{lonf:.8f},{hdg:.1f},{sats},{alt:.1f});}}catch(e){{document.getElementById('status').innerHTML='GPS '+{latf:.8f}.toFixed(6)+', '+{lonf:.8f}.toFixed(6);}}"
            self.map.page().runJavaScript(js)
        except Exception:
            pass
...
class TechnicalSetupWidget(QGroupBox):
    ...
    def __init__(self):
        ...
        # v102: use fixed FPS choices instead of a free spinbox.
        # Many capture cards behave better at a native timing mode; 50 FPS is useful for
        # PAL-style sources and can reduce the tiny periodic frame hiccup seen at 60 FPS.
        self.fps_spin = QComboBox(); self.fps_spin.addItems(["30", "50", "60"]); self.fps_spin.setCurrentText("50")
        ...
        btn_fpv_rec = QPushButton("Record Raw FPV")
    ...
    def build_ai_settings(self, ai_enabled: bool = False) -> AISettings:
        return AISettings(
            ...
            requested_fps=int(self.fps_spin.currentText()),
...
class DroneDashboardV23(QWidget):
    def __init__(self, theme: str = "dark", launch_fullscreen: bool = False, scale_mode: str = "Auto", cv_mode: str = "Live Computer Vision"):
        ...
        self.setWindowTitle("Drone Dashboard v112 - Smooth FPV + Stable GPS Drone Map")
        ...
        self.ui_timer = QTimer(self); self.ui_timer.setTimerType(Qt.CoarseTimer); self.ui_timer.timeout.connect(self.periodic_update); self.ui_timer.start(500)
        ...
        self.fpv_display_timer.setTimerType(Qt.PreciseTimer)
        self.fpv_display_timer.timeout.connect(self.update_fpv_display_from_thread)
        self.fpv_display_timer.setInterval(15)
        ...
        self.tx_hotplug_timer.start(5000)
    ...
    def watch_tx_hotplug(self):
        ...
        v112 smoothness fix: pygame/SDL joystick rescans can briefly stall the Qt
        event loop on some Windows laptops. Do not do HID rescans while FPV is
        running; users can still press Refresh TX manually in the Technical tab.
        """
    ...
    def start_fpv_ai(self, settings: AISettings):
        ...
        # v102: match the Qt display pull timer to the requested source FPS.
        # At 50 FPS this uses a 20 ms timer, reducing unnecessary UI wake-ups and
        # giving PAL/50Hz capture cards a cleaner cadence than a hard 15 ms loop.
        try:
            fps = max(1, int(settings.requested_fps))
            self.fpv_display_timer.setInterval(max(1, int(round(1000.0 / fps))))
        except Exception:
            self.fpv_display_timer.setInterval(20)
    ...
    def _settings_from_tech(self, ai_enabled: bool) -> AISettings:
        ...
        return AISettings(
            ...
            requested_fps=int(self.tech.fps_spin.currentText()),
            ai_enabled=bool(ai_enabled),
    ...
    def start_raw_fpv_recording(self):
        ...
        if self.fpv_thread is None:
            ...
            return
        try:
            path = self.fpv_thread.start_fpv_recording(self.tech.folder_line.text().strip() or self.record_base, fps=float(self.tech.fps_spin.currentText()))
            self.append_status(f"Raw FPV recording started: {path}")
        except Exception as e:
    ...
    def on_telemetry(self, d: Dict[str, Any]):
        ...
        # v102: main_map and mission_widget are normally the same compact widget.
        # Updating both doubled WebEngine JavaScript calls and could cause periodic FPV hitches.
        try:
            if self.mission_widget is not self.main_map:
                self.mission_widget.update_telemetry(d)
        except Exception:
            pass
        try:
            # v112 smoothness fix: WebEngine/Leaflet JavaScript updates can cause tiny
            # UI hitches. Keep GPS/map live only when the Main tab is visible.
            if self.tabs.currentWidget() is self.main_tab:
...
class BootLauncher(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Drone Dashboard v102 Boot")
        self.resize(760, 490)
        self.dashboard = None
        root = QVBoxLayout(self); root.setContentsMargins(32, 26, 32, 26); root.setSpacing(12)
        title = QLabel("Drone Dashboard v102")
